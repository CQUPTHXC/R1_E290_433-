...
struct RxDoneMsg
{
    uint8_t Payload[255];            // 接收数据缓冲区
    uint8_t PlhdPayload[16];         // 前导头接收缓冲区
	uint8_t TestModePayload[TEST_MODE_BUFFER_LEN];
    uint16_t PlhdSize;              // 前导头长度
    uint16_t Size;                  // 接收数据长度
    uint16_t mpam_recv_index;
    uint8_t mpam_recv_buf[1024];    // 根据实际应用设置缓冲区大小
    int8_t Rssi;
    float Snr;
};//RxDoneMsg.Size
#pragma pack ()
...
RF_Err_t rf_set_syncword(uint8_t sync);// 设置同步字
